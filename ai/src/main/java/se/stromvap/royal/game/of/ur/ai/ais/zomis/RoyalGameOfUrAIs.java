...
public class RoyalGameOfUrAIs {
    ...
    public static class URScorer implements Ai {
        ...
        public int myTurn(Game game) {
            ...
            ParamAndField<RoyalGameOfUr, Integer> best = ScoreUtils.pickBest(producer, zomisUR, new Random());
            int currentIndex = game.getStatus().getCurrentTurnPlayer() == game.getPlayer1() ? 0 : 1;
            int positionOfBest = zomisUR.getPieces()[currentIndex][best.getField()];
            return mapToZomisMove(zomisUR, game, positionOfBest);
        }

        private int mapToZomisMove(RoyalGameOfUr zomisUR, Game game, int positionOfBest) {
            ...
            if (positionOfBest == 0) {
                return game.getStatus().getCurrentTurnPlayer().getGamePieces().stream()
                        .filter(piece -> tiles.stream().noneMatch(tile -> tile.getGamePiece() == piece)).findAny().get().getId();
            }
            Tile tile = tiles.get(positionOfBest - 1);

            return tile.getGamePiece().getId();
        }
        ...
        private RoyalGameOfUr mapToZomis(Game game) {
            ...
            RoyalGameOfUr zomis = new RoyalGameOfUr(currentPlayer, game.getStatus().getLatestRoll(), pieces);
